...
func NewTestSnapshot() (clustersnapshot.ClusterSnapshot, error) {
	...
	return predicate.NewPredicateSnapshot(base.NewBasicSnapshotBase(), testFwHandle, true), nil
}
...
// NewCustomTestSnapshot returns an instance of ClusterSnapshot with a specific SnapshotBase that can be used in tests.
func NewCustomTestSnapshot(snapshot clustersnapshot.SnapshotBase) (clustersnapshot.ClusterSnapshot, error) {
	testFwHandle, err := framework.NewTestFrameworkHandle()
	if err != nil {
		return nil, err
	}
	return predicate.NewPredicateSnapshot(snapshot, testFwHandle, true), nil
}

// NewCustomTestSnapshotOrDie returns an instance of ClusterSnapshot with a specific SnapshotBase that can be used in tests.
func NewCustomTestSnapshotOrDie(t testFailer, snapshot clustersnapshot.SnapshotBase) clustersnapshot.ClusterSnapshot {
	result, err := NewCustomTestSnapshot(snapshot)
	if err != nil {
