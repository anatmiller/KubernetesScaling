...
// ResourceClaimId is an unique identifier for a ResourceClaim.
...
func (s Snapshot) AddClaims(newClaims []*resourceapi.ResourceClaim) error {
	for _, claim := range newClaims {
		if _, found := s.resourceClaimsById[ResourceClaimId{Name: claim.Name, Namespace: claim.Namespace}]; found {
			return fmt.Errorf("claim %s/%s already tracked in the snapshot", claim.Namespace, claim.Name)
		}
	}
	for _, claim := range newClaims {
		s.resourceClaimsById[ResourceClaimId{Name: claim.Name, Namespace: claim.Namespace}] = claim
	}
	return nil
}
...
func (s Snapshot) ReservePodClaims(pod *apiv1.Pod) error {
	...
	for _, claim := range claims {
		err := drautils.AddPodReservationIfNeededInPlace(claim, pod)
		if err != nil {
